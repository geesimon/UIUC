...
```{r}
f = medv ~ (log(crim) + zn + indus + chas + nox + rm + age + dis + rad + ptratio +
              black + log(lstat) + tax) ^ 2
...
**Solution**
We first check the multicollinearity.
```{r, message=FALSE}
library(readr)
bearing_data = read_csv("ballbearings.csv")
model = lm(log(L50) ~ . - Type - D, data = bearing_data)
vif(model)
```
...
bearing_data$Company = as.factor(bearing_data$Company)
#bearing_data$Type = as.factor(bearing_data$Type)
f = log(L50) ~ (log(P) + Z + D + Company) ^ 2
