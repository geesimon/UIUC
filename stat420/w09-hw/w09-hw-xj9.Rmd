...
Read data and fit a additive model with all predictors.
```{r, message=FALSE}
library(readr)
bearing_data = read_csv("ballbearings.csv")
vif(lm(log(L50) ~ ., data = bearing_data))
```
We can see `Company` has largest VIF value, we remove this predictor. Since `P` has large range, we log transfer this predictor to fit the response better. In addition. We make the full model as,
```{r}
...
all_models$adjr2 > 0.52
```
Start from size of 4, we have model with adjusted $R^2$ higher than 0.52.

We fit a model with the selected model.
```{r}
all_models$which[4,]
