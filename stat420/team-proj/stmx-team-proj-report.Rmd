...
##Predictor Selection
We select the significant predictors that have high correlation with SalePrice and store these predictor names to `Start_Pre_Names`.
...
#Store these significant predictor names 
...
loocv_rmse_model_int = lm(build_formula("Log_SalePrice", num_names, cat_names), 
                          data = house_data)
summary(loocv_rmse_model_int)$adj.r.squared
get_loocv_rmse(loocv_rmse_model_int)
```
...
|--------------|----------|-----------------|------------------------|-------------|-------------|
|Adjusted $R^2$|`r summary(full_model)$adj.r.squared`|`r summary(loocv_rmse_model)$adj.r.squared`|`r summary(loocv_rmse_model_int)$adj.r.squared`|`r summary(forward_aic_model)$adj.r.squared`|`r summary(forward_bic_model)$adj.r.squared`|
|AIC           |`r AIC(full_model)`|`r AIC(loocv_rmse_model)`|`r AIC(loocv_rmse_model_int)`|`r AIC(forward_aic_model)`|`r AIC(forward_bic_model)`|
|BIC           |`r BIC(full_model)`|`r BIC(loocv_rmse_model)`|`r BIC(loocv_rmse_model_int)`|`r BIC(forward_aic_model)`|`r BIC(forward_bic_model)`|
...
```{r}
test_sale_price = predict(full_model, newdata = test_data)
```
