...
```{r}
change_bad_names = function(data, name_pairs) {
  for (i in 1:length(name_pairs)) {
    colnames(house_data)[which(colnames(house_data) == name_pairs[i])] = names(name_pairs[i])
  }
}
BadNames =  c(FirstFlrSF = "1stFlrSF", SecondFlrSF = "2ndFlrSF", ThreeSsnPorch = "3SsnPorch")

change_bad_names(house_data, BadNames)
```
...
We load the data and do some pre-processing same as we did on trainig data.
```{r, message=FALSE}
...
change_names(test_data, BadNames)
