...
If we check the plot above, we can see the relation between each value of `OverallQual` and its mean of `Log_SalePrice` is quite linear. Increasing of 1 score of `OverallQual` has constant change on `Log_SalePrice`. Therefore, we can convert this categorical predictor to numerical predictor.
...
```
```{r}
partial_match = function(partial_names, full_names) {
  names = c()
  for(name in partial_names) {
    result = pmatch(name, full_names)
...
predictors  = partial_match(colnames(house_data), predictors)
...
vif_model = model_5
coef(vif_model)
summary(vif_model)$r.squared
```
```

Note: we could also remove the high VIF variable one by one. But per test, such approach won't give us better model than this one.
