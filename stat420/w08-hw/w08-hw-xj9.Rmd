...
coef(swiss_add)
```
We see after removal of the influential points, the coefficients are changed significantly.
...
The RMSEs of these two models show that the original model has better prediction. It is understandable because the original model take these removed points into account when calculate the coefficients.
...
We see the model with $\epsilon \sim N(0, \sigma = |x_1|)$ has larger proportion to reject the null hypothesis ($\beta_1 = 0$) at any $\alpha$ (0.01, 0.05, 0.10) and thus is less accurate on prediting the $\beta_1$ value.
...
- According to the Fitted versus Residuals Plot, at any fitted value, the mean of the residuals is not roughly 0. **Linearity assumption is invalid**.
