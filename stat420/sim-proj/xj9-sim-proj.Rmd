...
We will train the model with simulating data generated from sample data found in [`study_1.csv`](study_1.csv) and three possible levels of noise. Then predict $\hat{\text{E}}[Y]$ from $x_{i}$. 

- n = 15
- $\sigma \in (1, 5, 10)$
- $\hat{\text{E}}[Y \mid x_1 = -3, x_2 = 2.5, x_3 = 0.5, x_4 = 0]$, also as $\hat{y}(x_{eval})$
...
#### Simulation
We now perform the simulation `r num_sims` times for each $\sigma$. Each time, we update the $y$ variable in the data frame, leaving the $x$ variables the same. We then fit a model, and store $\hat{\beta}_1$, $s_e^2$ and compute $\hat{y}(x_{eval})$.
...
As we know, given a MLR model, we have

- $\text{SD}[\hat{\beta}_1] = s_e \sqrt{C_{22}}$
- $\text{SD}[\hat{y}(x_{eval})] = \sigma \sqrt{x_{eval}^\top\left(X^\top X\right)^{-1}x_{eval}}$
- $y(x_{eval}) = x_{eval}^\top\beta$


We compute $\text{SD}[{\beta_1}]$ and $\text{SD}[\hat{y}(x_{eval})]$ for each $\sigma$ for later comparison.
