...
Let's compute the **true** $\text{SD}[{\beta_1}]$, $\text{E}[\hat{y}(x_{eval})]$ and $\text{SD}[\hat{y}(x_{eval})]$ for each $\sigma$.
...
Let's plot the histograms of the empirical values, and overlay the true distributions if appropriate.
...
### Introduction
In this study, we will investigate the use of RMSE to select the best MLR model. We will see the minimum RMSE cannot garantee the best model and will also discuss the impact of different noise.
...
Therefore, we can conclude that **minimum RMSE cannot garantee to select the best model**.

### Discussion
**(A)** Does the method **always** select the correct model? On average, does is select the correct model?

**No.** As illustrated in our study, minimum RMSE won't always select the correct model especially when the RMSE among models are very close. On average, RMSE can help to select the correct model if the differences of RMSE are significant.
...
It may be obvious from the plot that for **train** data, the RMSE difference among models descrease when noise ($\sigma$) increase. However, it is not always the case for the test data. The max/min RMSE differences are,
...
### Introduction
In this study, we will exam how power of SLR model is affected by factors as,
...
```
#### Simulation
For each possible $\beta_1$ and $\sigma$ combination, we simulate from the true model 1000 times. Each time perform the significance of the regression test. 
```{r}
calc_power = function() {
...
- When $\sigma$ **increase**, power **decrease**. This also makes sense as $\sigma$ is considered as "noise", strong "noise" will bury  "signal" and thus make "signal" harder to be detected.  
...
Yes. Although more simulations usually give us less estimation error, we already see very stable and obvious trend from the output with 1000 simulations. Increase simulation times won't give us much benefit from this study. 

In another hand, reduce the simulation would make our conclusion not that obvious. For example, let's reduce the simulation to 100 and redraw the results.
...
According to the plots, we can find some bumps (especially when $\sigma$ is large and $\beta_1$ is small) which make our findings not that obvious.
